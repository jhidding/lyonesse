...
(library (lyonesse eager-comprehensions)

  (export do-ec list-ec append-ec string-ec string-append-ec vector-ec
          vector-of-length-ec sum-ec product-ec min-ec max-ec any?-ec every?-ec
          first-ec last-ec fold-ec fold3-ec : :list :string :vector :integers
          :range :real-range :char-range :port :dispatched :do :let :parallel
          :while :until :-dispatch-ref ; :-dispatch-set!
           ...
           (do-ec qualifier
                  (if (< i len)
                      (begin (vector-set! vec i expression)
                             (set! i (+ i 1)) )
                      (error 'vector-of-length-ec "vector is too short for the comprehension") ))
           (if (= i len)
               vec
               (error 'vector-of-length-ec "vector is too long for the comprehension") ))))))
...
#| CHANGELOG
 |   2016/10/22, by Johan Hidding
 |     - Merged with extra stream functions referenced in srfi.
 |     - Made into R6RS library form.
 |     - Errors into R6RS error forms.
 |     - Disabled :-dispatch-set!, can't set! exported variables.
 |#
